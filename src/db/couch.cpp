...
namespace
{
  std::string type2str(json_spirit::Value_type t)
  {
  ...
  struct cURL: boost::noncopyable
  {
    ...
    cURL() :
      curl_(curl_easy_init()), headers_(0),
  ...
  struct value_type_find
  {
    ...
    bool operator()(const json_spirit::Pair& v)
    {
  ...
  struct val2
  {
    T operator()(const json_spirit::Value& val)
    {
...
namespace couch
{
  struct Db::impl
  {
      ...
      switch (curl_.get_response_code())
        ...
        default:
          throw std::runtime_error(
                                   "Unexpected result: "
                                       + curl_.get_response_reason_phrase());
        }
    }
    ...
    void get(json_spirit::Value& val)
    {
      stream_.seekg(0);
  ...
  struct Document::impl
  {
    impl(const Db& db, const std::string& id) :
      id_(id), db_url_(db.url()), json_writer_(json_writer_stream_),
          json_reader_(json_reader_stream_)
    {
    }

    impl(const impl& rhs) :
      id_(rhs.id_), db_url_(rhs.db_url_), json_writer_(json_writer_stream_),
          json_reader_(json_reader_stream_)
    {
      obj_ = rhs.obj_;
    }
    ...
    void set_value(const std::string& key, const T& val)
    {
      ...
      std::pair<iterator, bool> it_inserted = obj_.insert(
                                                          std::make_pair(key,
                                                                         jval));
  ...
  private:
    ...
    void precondition_rev()
    {
      if (rev_.empty())
        throw std::runtime_error("The document must have a valid revision.");
    }
    void upload_json(const std::string& url, const std::string& request)
    {
      curl_.reset();
      json_writer_stream_.str("");
    ...
    inline std::string url_id()
    {
      return db_url_ + (id_.empty() ? "" : "/" + id_);
    }
    inline std::string url_id_rev()
    {
...
void couch::Db::all_dbs(const std::string& couch_db_root_url,
  ...
  if (curl.get_response_code() == cURL::OK)
    {
      ...
      std::transform(array.begin(), array.end(), std::back_inserter(dbs),
                     val2<std::string> ());
    }
  else
    throw std::runtime_error(
                             "Could not connect to couchdb at : "
                                 + couch_db_root_url);
}

bool couch::Db::create()
{
  return impl_->create();
}

void couch::Db::update_info()
{
  impl_->update_info();
}

void couch::Db::print_info(std::ostream & out)
{
  impl_->print_info(out);
}

std::string couch::Db::url() const
{
  return impl_->url_;
}
...
couch::Document& couch::Document::operator=(const Document& rhs)
{
  if (&rhs == this)
    return *this;
  if (impl_.get() == 0)
...
std::string couch::Document::toString()
{
  return impl_->toString();
}

void couch::Document::print_result(std::ostream & out)
{
  return impl_->print_result(out);
}

void couch::Document::update()
{
  impl_->update();
}

void couch::Document::create()
{
  impl_->create();
}

void couch::Document::commit()
{
  impl_->commit();
}

void couch::Document::clear_all()
{
  impl_->clear_all();
}

void couch::Document::clear_value(const std::string& key)
{
  impl_->clear_value(key);
}

void couch::Document::attach(const std::string& attachment_name,
                             std::istream & stream,
                             const std::string& content_type)
{
...
void couch::Document::get_attachment_stream(const std::string& attachment_name,
                                            std::ostream & stream)
{
...
template<>
bool couch::Document::get_value<bool>(const std::string& key)
{
  return impl_->get_value<bool> (key);
}
template<>
int couch::Document::get_value<int>(const std::string& key)
{
  return impl_->get_value<int> (key);
}
template<>
double couch::Document::get_value<double>(const std::string& key)
{
  return impl_->get_value<double> (key);
}
template<>
std::string couch::Document::get_value<std::string>(const std::string& key)
{
  return impl_->get_value<std::string> (key);
}

template<>
void couch::Document::set_value<bool>(const std::string& key, const bool& val)
{
  impl_->set_value(key, val);
}
template<>
void couch::Document::set_value<int>(const std::string& key, const int& val)
{
  impl_->set_value(key, val);
}
template<>
void couch::Document::set_value<double>(const std::string& key,
                                        const double& val)
{
  impl_->set_value(key, val);
}
...
template<>
void couch::Document::set_value<std::string>(const std::string& key,
                                             const std::string& val)
{
  impl_->set_value(key, val);
}

void couch::Document::set_value(const std::string& key, const char* val)
{
  impl_->set_value(key, val);
}

template<>
bool couch::Db::get_info_item<bool>(const std::string& item)
{
  return impl_->get_info_item<bool> (item);
}
template<>
int couch::Db::get_info_item<int>(const std::string& item)
{
  return impl_->get_info_item<int> (item);
}
template<>
double couch::Db::get_info_item<double>(const std::string& item)
{
  return impl_->get_info_item<double> (item);
}
template<>
std::string couch::Db::get_info_item<std::string>(const std::string& item)
{
...
void couch::View::add_map(const std::string & key, const std::string & function)
{
  map[key] = function;
}

void couch::View::add_reduce(const std::string & key,
                             const std::string & function)
{
  reduce[key] = function;
}

void couch::Db::run_view(View & v, int limit_rows, int start_offset,
                         int& total_rows, int& offset,
